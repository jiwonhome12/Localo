...
| 기능 | 설명 | 담당 |
|---|---|---|
| 지역 · 지원금 조회 | 지역 카드와 필터로 지역을 탐색, 지역 상세에서 축제와 지원금 정보를 확인. 지원금도 계산. | (배정 후 기입) |
| 축제 캘린더 · 티켓 연계 | 월별 축제 캘린더, 카테고리 필터·검색, 예매처 바로가기를 제공. | (배정 후 기입) |
| AI 코스 추천 | 여행 조건을 입력하면 AI가 여행 코스를 추천. | (배정 후 기입) |
